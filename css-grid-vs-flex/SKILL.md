...
上下两个元素之间的间距，**只由上面元素设 `margin-bottom`**，下面元素不设 `margin-top`。
...
```
CSS 版本
├── 现代（支持 gap）→ gap: 16px（最安全，无 margin 问题）
└── 旧版（需兼容）→
    ├── 统一用 margin-bottom + :last-child 清零
    └── 或 * + * { margin-top: X }
```

**你说的对：上下两个 div 之间的间距由上面的 div 控制，下面的 div 不设 margin-top。这是工程上更一致的做法。**
```
